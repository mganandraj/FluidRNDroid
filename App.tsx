...
let fluidInit = async () => {
  ...
  try {
    ...
    const url = "http://localhost:8081/abc";
    const documentId = "abc";
    const documentServiceFactory = getDocumentServiceFactory(documentId);
    const urlResolver = new MultiUrlResolver("http://localhost:8081/", documentId);

    let packageJson: IFluidPackage;
    let fluidModule: IFluidModule = {fluidExport: fluidExport};

    const resp = await fetch("http://localhost:3500/package.json", {
      headers: {
        'Cache-Control': 'no-cache'
      }
    });
    packageJson = await resp.json() as IFluidPackage;
    ...
    //const resp2 = await fetch("http://localhost:3500/dist/main.bundle.js");
    //const resp2js = await resp2.text();
    //const resp2module = await import(resp2js) ;
    // const resp2module = await import("http://localhost:3500/dist/main.bundle.js") ;
    // fluidModule = resp2module as IFluidModule;
    ...
    const routeOptions: ITinyliciousRouteOptions = {mode: "tinylicious", port: 8081};
...
class App extends React.Component {
  ...
  fluid = async function () {
    ...
    try {
      ...
      const url = "http://localhost:8081/abcdefg";
      const documentId = "abcdefg";
      const documentServiceFactory = getDocumentServiceFactory(documentId);
      const urlResolver = new MultiUrlResolver("http://localhost:8081/", documentId);
